...
void syn(double *t,int t_num,double *y,long y_num, double SI_th,double *T1map, double *Amap, double *Bmap,double *T1starmap, double *Errmap) 
		...
		if (SI_maxTI >SI_th)
        {
            for (jj=0; jj<8; jj++) {
            	...
            	for (kk=0;kk<t_num;kk++){
            		errd+=pow(y[kk+ii*t_num]-f(t[kk],par),2);
            	    }			
            	err[jj] =sqrt(errd);
            }
            minErr=err[0];
